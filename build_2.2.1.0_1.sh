...
if [ "$1" == "demo" ]; then
	...
	echo "Installing required OSPI libraries..."
	apt-get update
  
  # Install required packes using apt-get: don't try to install raspi-gpio if it isn't available via apt-get [install via github https://github.com/RPi-Distro/raspi-gpio instead] 
  if sudo apt-get --simulate install raspi-gpio; then  
    apt-get install -y libmosquitto-dev raspi-gpio libi2c-dev libssl-dev libgpiod-dev
  else 
    apt-get install -y libmosquitto-dev libi2c-dev libssl-dev libgpiod-dev
  fi
