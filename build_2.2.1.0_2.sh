...
#####################################################################################################################################################
## build.sh  V2.2.1.0_2 2024-08-29
##
...
# Script version info
script_version='V2.2.1.0_2';  ## 2024-08-29
package="jq"
...
while true; do
  ...
  if [[ $create_service_flag == "true" ]]; then
    ...
    if [[ ! -f "${start_up_directory%?}/${service_file_name}" ]]; then
      ...
      sudo echo "Description=OpenSprinkler Service V1.3 [OpenSprinkler V2.2.1.0 Aug 30, 2024]" >> "${start_up_directory%?}/${service_file_name}";
